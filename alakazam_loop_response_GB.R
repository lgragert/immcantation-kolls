...
#GB# removed sample_row as a parameter for the function, sicne it is never passed as an argument
Isotype_GeneUsage_Diversity <-function(BCRE_frame, name, subset_on){
  ...
  if (subset_on == "study_id"){
   # to seperate the patient ID based on sample type(pre,post or MBC) within the given Patient ID
    #done by specifying the columns to filter are sample type
    if (BCRE_frame$sample_type =='PBMC_Pre'[1]){ 
      sample_type <- 'PBMC_Pre'}
    if (BCRE_frame$sample_type == 'PBMC_Post'[1]) { 
      sample_type <- 'PBMC_Post'}
    if (BCRE_frame$sample_type== 'MBC'[1]) { 
      sample_type <- 'MBC'}

    ID_sample_name <- (paste0(name,"_", sample_type))
...
BCRE_subset_call <- function(BCRE_frame, subset_on=NULL, file_name="whole_frame"){
  if (!is.null(subset_on) & subset_on != "study_id"){ 
  ...
  } else if (subset_on == "study_id") {
    cols <- BCRE_frame[subset_on]
    cols <- unique(cols[[as.name(subset_on)]])
    for (unicorn in cols) {
      BCRE_subset <- filter(.data=BCRE_frame, !!as.name(subset_on)==unicorn)
      rows <- BCRE_subset["sample_id"]
      rows <- unique(rows[[as.name("sample_id")]])
      for (row in rows) {
        BCRE_sub2 <- filter(.data=BCRE_subset, !!as.name("sample_id")==row)
        Isotype_GeneUsage_Diversity(BCRE_subset, name=unicorn, subset_on)
      }
    }
  } else {
