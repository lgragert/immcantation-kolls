...
for (clone_id in IGHG_top_10$clone_id) {
  ...
  # convert to graphNEL object defined in graph package
  graphnel = as_graphnel(graph)
  ...
  graphnel_sub21 <- subGraph(c("Inferred21",), graphnel)
  ...
  # Select one tree for example purposes and make a list of example trees excluding multi-isotype trees: 
  # this piece doesn't work
  # graph_1 <- graph[[1]]
  
  # And add some annotation complexity to the tree
  # this only changes the labels for 2 and 7
  # sample_id actually contains the sequence_id - all sample_ids in this tree should be BCRE1
  # V(graph)$sample_id[c(2, 7)] <- "BCRE1"
  # changes the c_call for only 2 sequences - #2 and #7 to IGHG
  # these were previously NA probably because they were Inferred nodes?
  # V(graph)$c_call[c(2, 7)] <- "IGHG" 
  
  # graph_test <- V(graph)
  
  # this is subselecting graphs where there is a comma in C_CALL - where there is >1 isotype in the tree
  # we are already filtering to IGHG only
  ...
  # Add legend
  legend("topleft", c("Germline", "Inferred", "BCRE1"), 
  ...
  ### the section below doesn't work yet: I think it's related to lines 125-128
  
  # calculating subtree properties
  ### I don't know if it's correct to use "graph" as the input because it is a list

  # Set sample colors
  sample_colors <- c("BCRE1"="seagreen")
  
  # Box plots of node outdegree by sample
  p1 <- plotSubtrees(df, "name", "outdegree", colors=sample_colors, 
