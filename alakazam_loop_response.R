...
Isotype_GeneUsage_Diversity <-function(BCRE_frame, name, subset_on, sample_row){
  #Generate plot title variables to include description and subset 
  
  if (subset_on == "study_id"){
   # to seperate the patient ID based on sample type(pre,post or MBC) within the given Patient ID
   #!!!performs pre and post but no MBC with an error of >1 value which when troub le shooting lead me to the ifelse statements
     for (sample_type in BCRE_frame) {
      if (sample_type=='PBMC_Pre') {
        sample_row <- 'PBMC_Pre'
      } else if (sample_type=='PBMC_Post') {
        sample_row <- 'PBMC_Post'
      } else if (sample_type=='MBC') {
        sample_row <- 'MBC'
      }}
    # finishes without error but only with one sample type each
    # ifelse(sample_type=='PBMC_Pre',sample_row <- 'PBMC_Pre',TRUE )
    # ifelse(sample_type=='PBMC_Post',sample_row <- 'PBMC_Post',TRUE )
    # ifelse(sample_type=='MBC',sample_row <- 'MBC',TRUE )
    # }
    ID_sample_name <- (paste0(name,"_", sample_row))
    sample_name = paste0("PatientID",ID_sample_name)
    
    # to process all of a patients data together comment out to previous # and run the following line
    # sample_name = paste0("PatientID",name)
    
  }else if (subset_on == "sample_type"){
    sample_name = name
  }else if (subset_on == "sample_id"){
    sample_name = name
  }else if (subset_on == "study_type"){
...
# Generate function to define the subsets for analysis 
...
for (file in BCRE_files)
{
  ...
  BCRE['study_type'] = sample_row['Group']
  
  
# three separate if...then statements are needed for the sample times (pre and post desensitization
  #where MBC is taken pre) because the conditions, while mutually exclusive, are for different variables
  if (name == sample_row['Sample1ID']) {
    BCRE['sample_type'] = 'PBMC_Pre'
  } else if (name == sample_row['Sample2ID']) {
    BCRE['sample_type'] = 'PBMC_Post'
  } else if (name == sample_row['Bulk_Memory_B_Cell_Seq']) {
    BCRE['sample_type'] = 'MBC'    
  }
...
#seperate by treatment (Control,Sense,Desense)
BCRE_subset_call(BCRE_frame, subset_on="study_type", file_name="whole_frame")
#seperate by individual BCRE samples
BCRE_subset_call(BCRE_frame, subset_on="sample_id", file_name="whole_frame")
#seperate by pre or post treatment samples 
BCRE_subset_call(BCRE_frame, subset_on="sample_type", file_name="whole_frame")


#!!!sample row is a missing argument
# if(subset_on== "study_id"){
#   for (sample_type in BCRE_frame) {
#     if (sample_type=='PBMC_Pre') {
#       sample_row = 'PBMC_Pre'
